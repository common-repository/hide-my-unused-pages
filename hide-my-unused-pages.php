<?php 
/*
Plugin Name: Hide My Unused Pages
Plugin URI: http://wordpress.org/plugins/hide-my-unused-pages
Description: The widget Hide My Unused Pages changes the visibility of a blog after it has had no activity for a specified amount of time.
Author: sciencefreaks
Version: 1.0
Network: true
*/
...
class hide_unused_page {
	var $is_network;
	var $options;
	
	function __construct(){
		$this->is_network = (function_exists('is_plugin_active_for_network') ? is_plugin_active_for_network('hide-my-unused-pages/hide-my-unused-pages.php') : false);
		$this->options = ($this->is_network ? get_site_option('hide-my-unused-pages-options') : get_option('hide-inactives-site-options'));
		if(empty($this->options)){
			$this->options = array();
		}

        add_action('hide_unused_page_cron', array(&$this, 'cron'));
        add_action('hide_unused_page_warning', array(&$this, 'warning'));
	...
	function admin_menu(){
		if($this->is_network){
			add_submenu_page('settings.php', _('Hide My Unused Pages'), _('Hide My Unused Pages'), 'manage_sites', 'hide-my-unused-pages-options', array(&$this, 'plugin_options'));
		} else {
			add_options_page(_('Hide My Unused Pages'), _('Hide My Unused Pages'), 8, 'hide-my-unused-pages-options', array(&$this, 'plugin_options'));
		}
	}
    ...
    function cron(){
		...
		$query = apply_filters('hide_unused_page_edit_query', $query);
		...
		foreach($blogs as $blog){
		    ...
		    $warning_time = get_option('hide-my-unused-pages-warned', false);
		    ...
		    $args = apply_filters('hide_unused_page_update_blog', $args, $blog->blog_id);
		    ...
		    switch_to_blog($blog->blog_id);
		    delete_option('hide-my-unused-pages-warned');
		    switch_to_blog($blog_id);
    ...
    function get_inactivity_thresholds(){
	    ...
	    return apply_filters('hide-my-unused-pages-manage-inactivity-thresholds', $inactivity_thresholds);
    }
    ...
    function get_inactivity_warning_thresholds(){
	    ...
	    return apply_filters('hide-my-unused-pages-manage-inactivity-warning-thresholds', $inactivity_warning_thresholds);
    }
    ...
    function get_min_posts(){
        ...
        return apply_filters('hide-my-unused-pages-manage-min-posts', $min_posts);
    }
	...
	function get_update_frequencies(){
	    ...
	    return apply_filters('hide-my-unused-pages-manage-update-frequency', $update_frequencies);
	}
	
	function init(){
	    $timezone_offset = get_option('gmt_offset');
        
	    if(!wp_next_scheduled('hide_unused_page_warning') && !empty($this->options)){
		    // Schedule next run for warning message
		    wp_schedule_event(time() + $timezone_offset * 3600, $this->options['update_frequency'], 'hide_unused_page_warning');
		    
		    $this->warning();
		}
        
		if(!wp_next_scheduled('hide_unused_page_cron') && !empty($this->options)){
	        // Schedule next run
		    wp_schedule_event(time() + $timezone_offset * 3600, $this->options['update_frequency'], 'hide_unused_page_cron');
...
?>
	<div class="wrap">
		<h2><?php _e('Hide My Unused Pages')?></h2>
	<?php 
		$action = "";
			...
			case "update":
			    if(isset($_POST['update_frequency'])){
			        if($_POST['update_frequency'] != $this->options['update_frequency']){
    			        ...
    			        wp_clear_scheduled_hook('hide_unused_page_cron');
			        }
			    }
			    ...
			    if($this->is_network){
			    	update_site_option('hide-my-unused-pages-options', $this->options);
			    } else {
			    	update_option('hide-my-unused-pages-options', $this->options);
			    }
			    
			    $current_page = substr($_SERVER["SCRIPT_NAME"],strrpos($_SERVER["SCRIPT_NAME"],"/")+1);
    ?>
    		<script>
				window.location="<?php echo $current_page ?>?page=hide-my-unused-pages-options&updated=true&updatedmsg=<?php echo urlencode(__('Settings Saved')); ?>";
			</script>
    <?php
		    	break;
    ...
    ?>
    		<form method="post" action="<?php echo $current_page ?>?page=hide-my-unused-pages-options&action=update">
    		...
    		<tr valign="top">
    			...
    			<td>
    				<select name="update_frequency" id="update_frequency">
    				    ...
    				    <?php } ?>
    				    <?php do_action('hide-my-unused-pages-add-update-frequency', $this->options); ?>
    				</select>
    			</td>
    		</tr>
    		<tr valign="top">
    			...
    			<td>
    				<select name="inactivity_threshold" id="inactivity_threshold">
    				    ...
    				    <?php } ?>
    					<?php do_action('hide-my-unused-pages-add-inactivity-thresholds', $this->options); ?>
    				</select>
    			</td>
    		</tr>
    		<tr valign="top">
    			...
    			<td>
    				<select name="inactivity_warning_threshold" id="inactivity_warning_threshold">
    				    ...
    				    <?php } ?>
    					<?php do_action('hide-my-unused-pages-add-inactivity-warning-thresholds', $this->options); ?>
    				</select>
    			</td>
    		</tr>
    		<tr valign="top">
    		...
    		<tr valign="top">
    			...
    			<td>
    				<select name="min_posts" id="min_posts">
    				    ...
    				    <?php } ?>
    					<?php do_action('hide-my-unused-pages-add-min-posts', $this->options); ?>
    				</select>
    			</td>
    		</tr>
    		</table>
    		...
    		<?php settings_fields('hide-my-unused-pages_group'); ?>
...
<?php				
    ...
    function send_site_warning_email($site_details){
		...
		$admin_emails = apply_filters('hide_unused_page_edit_site_almost_hidden_to_emails', $admin_emails, $site_details);
		$headers = apply_filters('hide_unused_page_edit_site_almost_hidden_headers', $headers, $site_details);
		$subject = apply_filters('hide_unused_page_edit_site_almost_hidden_subject', $subject, $site_details);
		$message = apply_filters('hide_unused_page_edit_site_almost_hidden_message', $message, $site_details);
    ...
    function send_site_hidden_email($site_details){
		...
		$admin_emails = apply_filters('hide_unused_page_edit_site_hidden_to_emails', $admin_emails, $site_details);
		$headers = apply_filters('hide_unused_page_edit_site_hidden_headers', $headers, $site_details);
		$subject = apply_filters('hide_unused_page_edit_site_hidden_subject', $subject, $site_details);
		$message = apply_filters('hide_unused_page_edit_site_hidden_message', $message, $site_details);
	...
	function warning(){
		...
		$query = apply_filters('hide_unused_page_edit_warning_query', $query);
		$blogs = $wpdb->get_results($query);
		...
		foreach($blogs as $blog){
		    ...
		    if($warning_time === false){
    		    ...
    		    update_option('hide-my-unused-pages-warned', time() + $timezone_offset * 3600);
		    }
		}
	}
}

$hide_unused_page = new hide_unused_page();
?>